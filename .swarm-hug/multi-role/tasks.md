...
## Follow-up tasks (from sprint review)
- [A] (#13) Validate `juliet --role <name> <claude|codex>` with the existing role-name rules before building paths so invalid or traversal inputs (for example `../...`) are rejected with the invalid-name error instead of reading/writing outside `.juliet/<role>/`.
